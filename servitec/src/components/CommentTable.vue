...
<template>
    <div>
      <h2 class="text-center">Administrar Comentarios</h2>
      <table class="table">
        <thead>
          <tr>
        ...
        <tbody>
          <tr v-for="comment in comments" :key="comment.id">
            ...
            <td>
              <button class="btn btn-danger" @click="deleteComment(comment.id)">Eliminar</button>
            </td>
          </tr>
        </tbody>
      </table>
    ...
    methods: {
      ...
      deleteComment(id) {
        if (confirm('¿Estás seguro de que deseas eliminar este comentario?')) {
          axios
            .post('http://localhost/vue/php/delete_comment.php', { id })
            .then((response) => {
              if (response.data.success) {
                alert('Comentario eliminado correctamente');
                this.comments = this.comments.filter((comment) => comment.id !== id); // Elimina el comentario de la lista
              } else {
